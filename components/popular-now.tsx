...
export function PopularNow() {
  ...
  const shown = [...products].sort((left, right) => Number(Boolean(right.popular)) - Number(Boolean(left.popular)) || (right.ratingCount ?? 0) - (left.ratingCount ?? 0)).slice(0, 12);
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-11 sm:px-6 sm:py-14 lg:px-8">
      ...
      <div className="mt-6 grid grid-cols-3 gap-x-2.5 gap-y-5 sm:grid-cols-4 sm:gap-4 lg:grid-cols-6">{shown.map((product) => <ProductCard key={product.slug} product={product} />)}</div>
    </section>
  );
}
