...
import { BarChart3, Bell, CreditCard, FileText, KeyRound, Package, Settings, ShoppingCart, Users, Wallet, Ticket } from "lucide-react";
...
export function AdminBagusPayLayout({ children }: { children: React.ReactNode }) {
  return (
    <div className="grid min-h-[70vh] gap-3 lg:grid-cols-[210px_1fr]">
      <aside className="rounded-xl border border-white/10 bg-[#0d1019] p-2">
        ...
        <nav className="space-y-1">
          {menus.map(([label, Icon]) => (
            <button key={label} className="flex h-9 w-full items-center rounded-lg px-2 text-xs text-white/65 transition hover:bg-white/10 hover:text-white">
