...
export function AdminWalletManager({ view = "topups" }: { view?: "topups" | "checkout" }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const readiness = (data.gatewayReadiness as { ipaymu?: GatewayReadiness } | undefined)?.ipaymu;
      setGatewayReadiness(readiness ?? { ready: false, reason: "Status iPaymu tidak tersedia." });
    } catch (reason) {
  ...
  async function saveSettings() {
    ...
    try {
      ...
      setMessage("Pengaturan iPaymu berhasil disimpan.");
      setEditing(false);
      await load();
    } catch (reason) {
  ...
  if (view === "topups") {
    return <div className="space-y-3">
      <div><strong className="text-xs">Riwayat top up saldo</strong><p className="mt-1 text-[9px] text-white/30">Top up otomatis diproses melalui iPaymu.</p></div>
      {error && <div className="rounded-lg border border-red-400/20 bg-red-400/[0.05] p-3 text-xs text-red-200">{error}</div>}
      <div className="overflow-x-auto rounded-lg border border-white/[0.08]">
        <Table><TableHeader><TableRow className="border-white/[0.08] hover:bg-transparent"><TableHead className="text-[10px] text-white/35">Pelanggan</TableHead><TableHead className="text-[10px] text-white/35">Nominal</TableHead><TableHead className="text-[10px] text-white/35">Pembayaran</TableHead><TableHead className="text-[10px] text-white/35">Status</TableHead><TableHead className="text-[10px] text-white/35">Waktu</TableHead></TableRow></TableHeader><TableBody>{topups.length ? topups.map((item) => <TableRow key={item.id} className="border-white/[0.07]"><TableCell><strong className="text-xs">{item.customer_name}</strong><p className="text-[8px] text-white/25">{item.customer_email}</p></TableCell><TableCell className="text-xs font-bold text-[#d8ff8d]">{formatRupiah(item.amount)}</TableCell><TableCell className="text-[10px] text-white/45">{item.payment_method || "iPaymu"}</TableCell><TableCell><TopupStatus value={item.status} /></TableCell><TableCell className="text-[9px] text-white/30">{new Date(item.created_at).toLocaleString("id-ID")}</TableCell></TableRow>) : <TableRow><TableCell colSpan={5} className="py-8 text-center text-xs text-white/28">Belum ada riwayat top up.</TableCell></TableRow>}</TableBody></Table>
      </div>
    </div>;
  }

  const active = settings.ipaymuCheckoutEnabled || settings.ipaymuTopupEnabled;

  return <div className="space-y-3">
    {message && <div className="rounded-lg border border-[#b9ff35]/20 bg-[#b9ff35]/[0.05] p-3 text-xs text-[#d8ff8d]">{message}</div>}
    {error && <div className="rounded-lg border border-red-400/20 bg-red-400/[0.05] p-3 text-xs text-red-200">{error}</div>}
    <div className="overflow-x-auto rounded-lg border border-white/[0.08]">
      <Table><TableHeader><TableRow className="border-white/[0.08] hover:bg-transparent"><TableHead className="text-[10px] text-white/35">Payment gateway</TableHead><TableHead className="text-[10px] text-white/35">Status</TableHead><TableHead className="text-right text-[10px] text-white/35">Aksi</TableHead></TableRow></TableHeader><TableBody><TableRow className="border-white/[0.07]"><TableCell><strong className="text-xs">iPaymu</strong><p className="mt-0.5 text-[8px] text-white/28">Checkout {settings.ipaymuCheckoutEnabled ? "ON" : "OFF"} • Top up {settings.ipaymuTopupEnabled ? "ON" : "OFF"}</p>{active && !gatewayReadiness.ready && <p className="mt-1 max-w-md text-[8px] leading-3 text-amber-200/75">{gatewayReadiness.reason || "Konfigurasi iPaymu belum lengkap."}</p>}</TableCell><TableCell><span className={active ? gatewayReadiness.ready ? "text-[9px] font-bold text-[#d8ff8d]" : "text-[9px] font-bold text-amber-200" : "text-[9px] text-white/30"}>{active ? gatewayReadiness.ready ? "Siap" : "Belum siap" : "Nonaktif"}</span></TableCell><TableCell className="text-right"><Button type="button" onClick={() => setEditing(true)} variant="ghost" size="icon-sm" className="text-white/45 hover:text-white"><Edit3 className="size-3.5" /></Button></TableCell></TableRow></TableBody></Table>
    </div>

    <Dialog open={editing} onOpenChange={setEditing}>
      <DialogContent className="w-[calc(100vw-1rem)] max-w-lg overflow-x-hidden border-white/10 bg-[#10141d] text-white">
        <DialogHeader><DialogTitle>iPaymu</DialogTitle><DialogDescription className="text-white/38">Atur iPaymu untuk checkout dan top up otomatis.</DialogDescription></DialogHeader>
        <div className="space-y-3">
          <Toggle label="Aktif untuk checkout" value={settings.ipaymuCheckoutEnabled} onChange={(value) => setSettings((current) => ({ ...current, ipaymuCheckoutEnabled: value }))} />
          <Toggle label="Aktif untuk top up saldo" value={settings.ipaymuTopupEnabled} onChange={(value) => setSettings((current) => ({ ...current, ipaymuTopupEnabled: value }))} />
...
function MinTopup({ settings, setSettings }: { settings: WalletSettings; setSettings: React.Dispatch<React.SetStateAction<WalletSettings>> }) {
  return <Field label="Minimal top up saldo"><Input type="number" min={1000} value={settings.minTopup} onChange={(e) => setSettings((current) => ({ ...current, minTopup: Number(e.target.value) }))} className="admin-input" /></Field>;
}
