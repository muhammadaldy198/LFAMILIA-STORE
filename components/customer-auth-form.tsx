...
export function CustomerAuthForm({
  ...
  useEffect(() => {
    ...
    void fetch("/api/auth/google/status", { cache: "no-store" })
      .then((response) => response.ok ? response.json() : Promise.resolve({ enabled: false }))
      .then((payload: { enabled?: boolean }) => {
        if (!cancelled) setGoogleReady(Boolean(payload.enabled));
      })
      .catch(() => {
    ...
    <section className="mx-auto w-full max-w-[560px] rounded-[24px] border border-white/[0.09] bg-[#090c14]/95 p-5 shadow-[0_24px_80px_rgba(0,0,0,0.36)] sm:p-7">
      ...
      <form onSubmit={submit} className="mt-6 space-y-4">
        ...
        <AuthField label="Password" icon={<LockKeyhole className="size-[18px]" />}>
          <input
            ...
            placeholder={mode === "login" ? "Masukkan kata sandi Anda" : "Minimal 8 karakter"}
            className="h-12 w-full bg-transparent pl-11 pr-12 text-sm text-white outline-none placeholder:text-white/24"
          />
          <button
            type="button"
            onClick={() => setShowPassword((value) => !value)}
            aria-label={showPassword ? "Sembunyikan password" : "Tampilkan password"}
            className="absolute bottom-0 right-0 grid h-12 w-12 place-items-center text-white/40 transition hover:text-white/75"
          >
        ...
        {mode === "register" && (
          <label className="flex cursor-pointer items-start gap-3 pt-1 text-[11px] leading-5 text-white/45">
            <input
              ...
              className="mt-1 size-4 shrink-0 accent-[#b9ff35]"
            />
            <span>
        ...
        {error && (
          <p className="rounded-xl border border-red-400/20 bg-red-400/[0.06] px-3 py-2.5 text-[11px] leading-5 text-red-200">
            {error}
          </p>
        )}
        ...
        <Button
          disabled={saving}
          className="h-12 w-full rounded-xl bg-[#b9ff35] text-sm font-black text-[#091006] hover:bg-[#c7ff58]"
        >
      ...
      <div className="mt-5 text-center text-[11px] text-white/45">
        ...
        <button
          ...
          className="font-black text-[#cfff72] hover:underline"
        >
      ...
      {googleReady && (
        <>
          ...
          <a
            ...
            className="flex h-12 w-full items-center justify-center rounded-xl border border-white/[0.1] bg-white/[0.025] text-sm font-bold text-white/80 transition hover:bg-white/[0.06] hover:text-white"
          >
            <span className="mr-2 grid size-6 place-items-center rounded-full bg-white text-[13px] font-black text-[#4285f4]">G</span>
            Masuk dengan Google
          </a>
        </>
      )}
  ...
  return (
    <label className="block">
      ...
      <span className="relative block overflow-hidden rounded-xl border border-white/[0.09] bg-[#10131b] transition focus-within:border-[#b9ff35]/45 focus-within:ring-2 focus-within:ring-[#b9ff35]/10">
        <span className="pointer-events-none absolute bottom-0 left-0 grid h-12 w-11 place-items-center text-white/36">
          {icon}
        </span>
        {children}
      </span>
