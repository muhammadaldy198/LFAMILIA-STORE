...
export function AdminIntegrationWorkspace() {
  ...
  return <div>
    ...
    {tab === "Ringkasan" && <div className="grid grid-cols-2 gap-4"><Card icon={<Network className="size-5" />} title="Digiflazz" ready={isConfigured("digiflazz", digiflazzEnvironment)} onClick={() => setTab("Digiflazz")} /><Card icon={<KeyRound className="size-5" />} title="KokinPay" ready={isConfigured("kokinpay", "global")} onClick={() => setTab("KokinPay")} /><Card icon={<LogIn className="size-5" />} title="Google Login" ready={isConfigured("google", "global")} onClick={() => setTab("Google Login")} /><Card icon={<Mail className="size-5" />} title="Resend Email" ready={isConfigured("resend", "global")} onClick={() => setTab("Resend Email")} /><Card icon={<Server className="size-5" />} title="VPS Relay" ready={isConfigured("relay", "global")} onClick={() => setTab("Relay & Keamanan")} /><Panel title="Callback Digiflazz" description="Tempel URL ini pada pengaturan webhook Digiflazz." className="col-span-2"><div className="p-4"><CopyUrl label="Digiflazz Webhook URL" value={digiflazzWebhook} /></div></Panel></div>}
    ...
    {tab === "Google Login" && <Panel title="Google Login" description="Google Identity Services untuk masuk/daftar pelanggan. Hanya Client ID yang dibutuhkan; tidak memakai Client Secret atau redirect URI."><div className="grid gap-4 p-4"><Text label="Client ID" value={values.googleClientId || ""} onChange={(value) => setValue("googleClientId", value)} /><div className="rounded-md border border-blue-100 bg-blue-50 px-3 py-2 text-[9px] leading-4 text-blue-700">Di Google Cloud, gunakan OAuth client tipe Web application dan tambahkan domain production pada Authorized JavaScript origins. Contoh: https://lfamiliastore.my.id</div></div></Panel>}
