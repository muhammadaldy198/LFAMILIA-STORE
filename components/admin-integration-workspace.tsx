...
export function AdminIntegrationWorkspace() {
  ...
  async function save() {
    ...
    try {
      if (tab === "DOKU Direct API") {
        await saveProfile("doku", "direct", dokuEnvironment, { clientId: values.dokuClientId || "", secretKey: values.dokuSecretKey || "", privateKey: values.dokuPrivateKey || "", apiUrl: values.dokuApiUrl || "", qrisMerchantId: values.dokuMerchantId || "", qrisTerminalId: values.dokuTerminalId || "", qrisPostalCode: values.dokuPostalCode || "", vaConfigJson: values.dokuVaConfig || "" });
      ...
      setValues((current) => ({ ...current, dokuClientId: "", dokuSecretKey: "", dokuPrivateKey: "", digiflazzApiKey: "", digiflazzWebhookSecret: "", melostoreApiKey: "", melostoreSecretKey: "", melostoreNicknameApiKey: "", resendApiKey: "", relayToken: "", voucherEncryptionKey: "" }));
    } catch (reason) {
  ...
  async function testCurrent() {
    ...
    try {
      if (tab === "Relay & Keamanan") {
        ...
        const target = tab === "DOKU Direct API" ? ["doku", dokuEnvironment] : tab === "Digiflazz" ? ["digiflazz", digiflazzEnvironment] : tab === "Melostore Nickname" ? ["melostore", "global"] : tab === "Resend Email" ? ["resend", "global"] : null;
  ...
  return <div>
    ...
    {tab === "DOKU Direct API" && <TwoColumn main={<Panel title="Kredensial DOKU Direct API" description="Field rahasia dikosongkan kembali setelah tersimpan dan tidak pernah ditampilkan oleh API." action={<SecretToggle show={showSecrets} onClick={() => setShowSecrets((current) => !current)} />}><div className="grid grid-cols-2 gap-4 p-4">
      ...
      <Field label="RSA Private Key (PKCS#8)" help="Disimpan terenkripsi dan tidak pernah dikirim ke frontend." wide><textarea value={values.dokuPrivateKey || ""} onChange={(event) => setValue("dokuPrivateKey", event.target.value)} className={`${inputClass} h-24 py-2 font-mono`} placeholder={profileConfigured("doku", dokuEnvironment) ? "Tersimpan — isi hanya untuk mengganti" : "-----BEGIN PRIVATE KEY-----"} /></Field>
...
function SecurityPanel() { return <Panel title="Pengamanan" description="Aturan backend aktif."><div className="space-y-3 p-4"><SecurityLine text="Credential terenkripsi di database" /><SecurityLine text="Private key tidak dikirim ke browser" /><SecurityLine text="Signature callback diverifikasi" /><SecurityLine text="Idempotency mencegah saldo ganda" /></div></Panel>; }
