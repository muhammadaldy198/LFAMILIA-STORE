...
function ProductMediaField({ label, field, value, onChange, uploading, onUpload }: { label: string; field: "imageUrl" | "bannerUrl"; value: string; onChange(value: string): void; uploading: boolean; onUpload(field: "imageUrl" | "bannerUrl", file?: File): void }) {
  return <label className="text-[8px] font-bold text-[#3d4f68]">{label}<input value={value} onChange={(event) => onChange(event.target.value)} placeholder="URL gambar (opsional)" className="mt-[4px] h-[34px] w-full rounded-[4px] border border-[#dce3eb] px-[9px] text-[8px] outline-none placeholder:text-[#929eae] focus:border-[#2580eb]" /><span className="mt-[6px] inline-flex h-[30px] cursor-pointer items-center gap-[6px] rounded-[4px] border border-[#cfd9e5] bg-[#f8fbff] px-[10px] text-[8px] font-bold text-[#0875ed]"><Upload className="size-[11px]" />{uploading ? "Mengunggah..." : "Pilih & Unggah Foto"}<input type="file" accept="image/png,image/jpeg,image/webp,image/gif" className="sr-only" disabled={uploading} onChange={(event) => onUpload(field, event.target.files?.[0])} /></span><small className="mt-[4px] block font-normal text-[#718198]">JPG, PNG, WEBP, GIF · Maks. 6MB</small></label>;
}
