...
export function ContactPanel() {
  ...
  async function copyMessage(event: FormEvent) {
    ...
    const text = `Halo LFAMILIA STORE\nNama: ${name}\nInvoice: ${invoice || "-"}\nPesan: ${message}`;
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-[.85fr_1.15fr]">
      <div className="space-y-3">
        {[{ icon: MessageCircle, title: "WhatsApp", text: "Nomor dukungan akan ditambahkan." }, { icon: Camera, title: "Instagram", text: "Akun resmi akan ditambahkan." }, { icon: Mail, title: "Email", text: "Alamat bantuan akan ditambahkan." }].map(({ icon: Icon, title, text }) => <div key={title} className="flex items-center gap-4 rounded-2xl border border-white/[0.08] bg-[#10131b] p-5"><span className="grid size-11 shrink-0 place-items-center rounded-xl bg-[#b9ff35]/[0.09] text-[#cfff72]"><Icon className="size-5" /></span><div><h2 className="text-sm font-black">{title}</h2><p className="mt-1 text-xs text-white/34">{text}</p></div></div>)}
        <div className="flex items-start gap-3 rounded-2xl border border-white/[0.08] bg-white/[0.02] p-5"><Clock3 className="mt-0.5 size-4 text-[#b9ff35]" /><p className="text-xs leading-5 text-white/38">Dukungan pelanggan disiapkan untuk menangani pertanyaan produk dan pesanan.</p></div>
      </div>

      <form onSubmit={copyMessage} className="rounded-[26px] border border-white/[0.09] bg-[#10131b] p-5 sm:p-8">
        <p className="eyebrow">Siapkan pesan</p><h2 className="text-xl font-black">Ceritakan kendalamu</h2><p className="mt-2 text-xs leading-5 text-white/34">Isi formulir, salin pesannya, lalu kirim melalui kanal bantuan setelah diaktifkan.</p>
