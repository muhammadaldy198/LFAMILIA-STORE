...
function editPayload(kind: Exclude<ContentKind, "review">, raw: NonNullable<Banner["raw"]> | NonNullable<MiniItem["raw"]>, data: FormData, title: string) {
  ...
  if (kind === "banner") return { ...(raw as BannerPayload), title, subtitle: text("subtitle"), imageUrl: text("imageUrl", (raw as BannerPayload).imageUrl), mobileImageUrl: text("mobileImageUrl"), ctaLabel: text("ctaLabel", "Lihat Sekarang"), ctaHref: text("href", "/"), showDesktop: data.get("showDesktop") === "on", showMobile: data.get("showMobile") === "on", sortOrder: order, isActive: active };
  ...
  return (
    <aside className="sticky top-[70px] self-start overflow-hidden rounded-[7px] border border-[#dfe6ef] bg-white shadow-[0_8px_24px_rgba(18,35,60,.08)]">
      ...
      <form key={`${editor.kind}-${editor.id}`} onSubmit={onSubmit} className="p-[13px]">
        ...
        <EditorField label={editor.kind === "faq" ? "Pertanyaan" : editor.kind === "review" ? "Pelanggan" : "Judul"} name="title" defaultValue={banner?.title ?? item?.title ?? ""} readOnly={editor.kind === "review"} />
        {editor.kind === "banner" && <>
          <EditorTextArea label="Subjudul" name="subtitle" defaultValue={banner?.raw?.subtitle ?? ""} />
          ...
          <EditorField label="Teks tombol" name="ctaLabel" defaultValue={banner?.raw?.ctaLabel ?? "Lihat Sekarang"} />
          <EditorField label="Link Tujuan" name="href" defaultValue={banner?.raw?.ctaHref ?? "/"} />
...
function BannerArtwork({ banner, large }: { banner: Banner; large?: boolean }) { const imageUrl = banner.raw?.imageUrl; return <div className={`relative overflow-hidden rounded-[4px] bg-gradient-to-r ${banner.tone} text-white ${large ? "h-full w-full" : "h-[38px] w-[108px]"}`}>{imageUrl && <img src={imageUrl} alt="" className="absolute inset-0 size-full object-cover" />}<div className={`absolute inset-0 bg-[linear-gradient(90deg,rgba(3,12,30,.86),rgba(3,12,30,.28))] ${large ? "p-[13px]" : "p-[5px]"}`}><strong className={`block max-w-[72%] font-black leading-[1.05] ${large ? "text-[12px]" : "text-[6px]"}`}>{banner.title.toUpperCase()}</strong><small className={`font-bold text-[#dcff35] ${large ? "text-[7px]" : "text-[4px]"}`}>{banner.raw?.subtitle || "PROSES CEPAT & AMAN"}</small></div></div>; }
