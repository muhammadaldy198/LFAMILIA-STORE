  ...
  return <div className="space-y-6">
    ...
    <ManagedSection icon={Megaphone} title="Banner carousel Home" description="Tambahkan sebanyak yang dibutuhkan. Banner akan berganti otomatis di bagian paling atas Home." onAdd={() => setBanners((current) => [...current, emptyBanner(current.length)])}>
      {banners.length ? banners.map((item, index) => <div key={item.id ?? `banner-${index}`} className="rounded-2xl border border-white/[0.08] bg-white/[0.02] p-4">
        <div className="grid gap-4 lg:grid-cols-[280px_1fr]"><AdminMediaUpload label="Gambar banner" value={item.imageUrl} onChange={(value) => updateAt(setBanners, index, { imageUrl: value })} help="Rekomendasi rasio 16:7 atau 16:9." /><div className="grid gap-3 sm:grid-cols-2"><Field label="Judul" wide><Input value={item.title} onChange={(event) => updateAt(setBanners, index, { title: event.target.value })} className="admin-input" /></Field><Field label="Subjudul" wide><Textarea value={item.subtitle} onChange={(event) => updateAt(setBanners, index, { subtitle: event.target.value })} className="min-h-20 rounded-xl border-white/10 bg-white/[0.025] text-xs text-white" /></Field><Field label="Teks tombol"><Input value={item.ctaLabel} onChange={(event) => updateAt(setBanners, index, { ctaLabel: event.target.value })} className="admin-input" /></Field><Field label="Tujuan tombol"><Input value={item.ctaHref} onChange={(event) => updateAt(setBanners, index, { ctaHref: event.target.value })} className="admin-input" placeholder="/catalog atau URL" /></Field><Field label="Urutan"><Input type="number" min={0} value={item.sortOrder} onChange={(event) => updateAt(setBanners, index, { sortOrder: Number(event.target.value) })} className="admin-input" /></Field><label className="flex items-center justify-between rounded-xl border border-white/[0.08] px-3 text-xs text-white/55"><span>Aktif</span><Switch checked={item.isActive} onCheckedChange={(checked) => updateAt(setBanners, index, { isActive: checked })} /></label></div></div>
...
function emptyBanner(sortOrder: number): HomeBannerRecord { return { id: null, title: "", subtitle: "", imageUrl: "", ctaLabel: "Lihat produk", ctaHref: "#produk", isActive: true, sortOrder }; }
