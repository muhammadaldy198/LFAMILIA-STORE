...
export function AdminStorefrontManager({ role }: { role: "owner" | "staff" }) {
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-2xl border border-white/[0.08] bg-white/[0.015] p-4 sm:p-5"><div className="flex items-start justify-between gap-4"><div><h3 className="font-bold">Identitas toko, banner cadangan & kanal bantuan</h3><p className="mt-1 text-[10px] text-white/30">Semua teks, media utama, dan kontak publik dapat diubah tanpa menyentuh kode.</p></div><Button onClick={() => void saveSettings()} disabled={saving === "settings"} className="rounded-xl bg-[#b9ff35] text-xs font-black text-[#091006] hover:bg-[#d0ff75]">{saving === "settings" ? <LoaderCircle className="mr-2 size-4 animate-spin" /> : <Save className="mr-2 size-4" />}Simpan</Button></div><div className="mt-5 grid gap-4 sm:grid-cols-2">
      ...
      <label className="flex items-center justify-between rounded-xl border border-white/[0.08] bg-white/[0.02] p-3 text-xs text-white/60 sm:col-span-2"><span>Tampilkan banner cadangan Home</span><Switch checked={settings.bannerEnabled} onCheckedChange={(checked) => updateSetting("bannerEnabled", checked)} /></label>
      ...
      <div className="sm:col-span-2"><AdminMediaUpload label="Gambar banner cadangan Home" value={settings.bannerImageUrl} onChange={(value) => updateSetting("bannerImageUrl", value)} help="Dipakai hanya jika Banner carousel Home belum memiliki banner aktif." /></div>
