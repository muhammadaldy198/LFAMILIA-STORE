...
export function AdminStorefrontManager({ role }: { role: "owner" | "staff" }) {
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-2xl border border-white/[0.08] bg-white/[0.015] p-4 sm:p-5"><div className="flex items-start justify-between gap-4"><div><h3 className="font-bold">Identitas toko & banner Home</h3><p className="mt-1 text-[10px] text-white/30">Semua teks dan media utama dapat diubah tanpa menyentuh kode.</p></div><Button onClick={() => void saveSettings()} disabled={saving === "settings"} className="rounded-xl bg-[#b9ff35] text-xs font-black text-[#091006] hover:bg-[#d0ff75]">{saving === "settings" ? <LoaderCircle className="mr-2 size-4 animate-spin" /> : <Save className="mr-2 size-4" />}Simpan</Button></div><div className="mt-5 grid gap-4 sm:grid-cols-2">
      ...
      <Field label="URL logo" wide><Input type="url" value={settings.logoUrl ?? ""} onChange={(event) => updateSetting("logoUrl", event.target.value)} placeholder="https://.../logo.png" className="admin-input" /></Field>
      ...
      <Field label="URL gambar banner"><Input type="url" value={settings.bannerImageUrl ?? ""} onChange={(event) => updateSetting("bannerImageUrl", event.target.value)} className="admin-input" placeholder="https://.../banner.webp" /></Field>
    ...
    </div>{settings.bannerImageUrl ? <div className="mt-4 overflow-hidden rounded-2xl border border-white/10"><div className="h-36 bg-cover bg-center" style={{ backgroundImage: `linear-gradient(90deg, rgba(8,11,20,.9), rgba(8,11,20,.15)), url(${JSON.stringify(settings.bannerImageUrl).slice(1, -1)})` }} /></div> : <div className="mt-4 grid h-28 place-items-center rounded-2xl border border-dashed border-white/10 text-white/20"><ImageIcon className="size-6" /></div>}</section>
