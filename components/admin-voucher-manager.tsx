...
import { Copy, Edit3, Eye, KeyRound, LoaderCircle, Mail, PackagePlus, RefreshCw, RotateCcw } from "lucide-react";
...
type VoucherConfig = { encryptionReady: boolean; emailReady: boolean; deliveryChannel: string; };
type DashboardResponse = { stocks?: Stock[]; deliveries?: Delivery[]; config?: VoucherConfig; error?: string; };
const emptyConfig: VoucherConfig = { encryptionReady: false, emailReady: false, deliveryChannel: "website" };
...
export function AdminVoucherManager() {
  ...
  return <div className="space-y-4">
    ...
    <div className="flex flex-wrap gap-x-4 gap-y-1 text-[9px] text-white/30"><Status ready={config.encryptionReady} icon={KeyRound} label="Enkripsi"/><Status ready={config.emailReady} icon={Mail} label="Resend"/><span>Pengiriman kode otomatis lewat website{config.deliveryChannel === "email" ? " + email" : ""}</span></div>
