...
export function AdminHostedGatewayIntegration({ provider }: { provider: Provider }) {
  ...
  return <div className="grid grid-cols-[minmax(0,1fr)_360px] gap-4">
    ...
    <div className="space-y-4">
      <Panel title="Status Pemakaian" description="Mode aktif diatur dari menu Pembayaran → Metode Pembayaran."><div className="space-y-3 p-4 text-[9px] text-[#42516a]"><div className="flex items-center justify-between"><span>Mode aktif</span><strong>{activeMode === (isDoku ? "checkout" : "snap") ? title : isDoku ? "DOKU Direct API" : "Midtrans BI-SNAP"}</strong></div><div className="flex items-center justify-between"><span>Environment aktif</span><strong className="capitalize">{activeEnvironment || "sandbox"}</strong></div><div className="flex items-center justify-between"><span>{environment} credential</span><Status tone={configured ? "green" : "amber"}>{configured ? "Siap" : "Belum siap"}</Status></div></div></Panel>
