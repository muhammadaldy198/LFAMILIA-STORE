    ...
    <section className="relative overflow-hidden border-b border-white/[0.07]">
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
        <div className="relative overflow-hidden rounded-[28px] border border-white/[0.09] bg-[#10131b] p-6 sm:p-9 lg:grid lg:min-h-[430px] lg:grid-cols-[1.1fr_.9fr] lg:items-center lg:gap-10 lg:p-12">
          ...
          <div className="relative z-10 mt-10 hidden justify-end lg:flex"><div className="w-72 rounded-[28px] border border-white/10 bg-black/35 p-5 backdrop-blur-md"><p className="text-[9px] uppercase tracking-[0.2em] text-white/35">Layanan LFAMILIA</p><div className="mt-5 space-y-3">{[["Top Up Game", "Proses otomatis"], ["Voucher Digital", "Pengiriman kode"], ["Flash Sale", "Promo terjadwal"], ["Cek Transaksi", "Status real-time"]].map(([title, detail]) => <div key={title} className="rounded-2xl border border-white/[0.08] bg-white/[0.04] p-4"><strong className="text-xs">{title}</strong><span className="mt-1 block text-[9px] text-white/35">{detail}</span></div>)}</div></div></div>
        </div>
      </div>
    </section>
  );
