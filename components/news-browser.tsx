...
export function NewsBrowser() {
  ...
  useEffect(() => { void fetch("/api/news", { cache: "no-store" }).then((response) => response.json().catch(() => ({}))).then((data: { articles?: NewsRecord[] }) => setArticles(data.articles ?? [])).catch(() => setArticles([])).finally(() => setLoading(false)); }, []);
