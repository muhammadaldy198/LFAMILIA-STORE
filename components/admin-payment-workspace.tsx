...
type GatewayReadiness = {
  doku?: { ready?: boolean; configured?: boolean; reason?: string | null; environment?: string | null; mode?: string };
  midtrans?: { ready?: boolean; configured?: boolean; reason?: string | null; missing?: string[]; environment?: string | null; relayReady?: boolean; mode?: string };
};
...
function GatewayControl({ title, ready, configured, enabled, environment, onEnabled, onEnvironment }: { gateway: Gateway; title: string; ready: boolean; configured: boolean; enabled: boolean; environment: Environment; onEnabled(value: boolean): void; onEnvironment(value: Environment): void }) {
  return <div className="rounded-md border border-[#e3e8ef] p-3">
    <div className="mb-3 flex items-center justify-between"><div><strong className="block text-[10px] text-[#34445f]">{title}</strong><Status tone={ready || configured ? "green" : "amber"}>{ready ? "Konfigurasi Siap" : configured ? `Konfigurasi ${environment === "sandbox" ? "Sandbox" : "Production"} Tersimpan` : "Belum diisi"}</Status></div><div className="flex items-center gap-2"><span className="text-[8px] font-semibold text-[#718198]">Aktif</span><Toggle checked={enabled} onChange={onEnabled} /></div></div>
