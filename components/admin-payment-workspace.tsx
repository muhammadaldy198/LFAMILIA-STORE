...
type GatewayReadiness = {
  doku?: { ready?: boolean; reason?: string | null; environment?: string | null };
  midtrans?: { ready?: boolean; missing?: string[]; environment?: string | null; relayReady?: boolean };
};
...
export function AdminPaymentWorkspace() {
  ...
  const load = useCallback(async () => {
    ...
    const [channelPayload, pagePayload, walletPayload, orderPayload] = await Promise.all([channelResponse.json(), pageResponse.json(), walletResponse.json(), orderResponse.json()]) as [
      { error?: string; channels?: Array<{ id: number | null; method: "va" | "ewallet" | "qris"; channel: string; name: string; description: string; imageUrl?: string; isActive: boolean; sortOrder: number; gateway: Gateway; gatewayConfig?: Record<string, string> }>; gatewaySettings?: GatewaySetting[]; gatewayReadiness?: GatewayReadiness },
    ...
    if (channelPayload.channels) setChannels(channelPayload.channels.map((item) => ({
      ...
      group: item.method === "qris" ? "QRIS" : item.method === "va" ? "VA Bank" : "E-Wallet",
  ...
  async function syncChannels() {
    ...
    try {
      ...
      await load();
      setMessage("Daftar metode resmi LFAMILIA berhasil disinkronkan. Channel baru tetap OFF sampai diaktifkan manual.");
  ...
  async function testConnection() {
    ...
    try {
      ...
      const doku = readiness?.doku?.ready ? "DOKU siap" : "DOKU belum siap";
      const midtrans = readiness?.midtrans?.ready ? "Midtrans siap" : "Midtrans belum siap";
      setMessage(`${doku} · ${midtrans}. Uji transaksi nyata tetap dilakukan pada tahap pra-peluncuran.`);
  ...
  return <div>
    ...
    {tab === "Metode Pembayaran" && <div className="grid grid-cols-[minmax(0,1fr)_340px] gap-4">
      <Panel title="Metode Pembayaran" description="Gateway hanya terlihat oleh Admin. Customer melihat nama bank, QRIS, atau e-wallet." action={<button type="button" onClick={syncChannels} disabled={busy} className={buttonClass}><RefreshCw className="size-3.5" />Sinkron Metode</button>}>
        <table className="w-full text-left"><thead className="bg-[#f6f8fb] text-[8px] uppercase text-[#718198]"><tr><th className="px-4 py-2.5">Channel</th><th>Jenis</th><th>Gateway</th><th>Kesiapan</th><th>Status</th><th className="pr-4 text-right">Aksi</th></tr></thead>
          <tbody className="divide-y divide-[#edf0f4]">{channels.map((channel) => <tr key={`${channel.method}:${channel.id}`} className="text-[9px] text-[#42516a]"><td className="px-4 py-2.5"><div className="flex items-center gap-2"><span className="grid size-7 place-items-center rounded bg-blue-50 text-[#0769e9]">{channel.group === "QRIS" ? <QrCode className="size-3.5" /> : channel.group === "VA Bank" ? <Landmark className="size-3.5" /> : <Smartphone className="size-3.5" />}</span><div><strong className="block text-[#23334e]">{channel.name}</strong><span className="text-[8px] text-[#8a98aa]">{channel.description}</span></div></div></td><td>{channel.group}</td><td><Status tone={channel.gateway === "midtrans" ? "amber" : "green"}>{channel.gateway === "midtrans" ? "Midtrans" : "DOKU"}</Status></td><td>{channel.gateway === "midtrans" ? (channel.gatewayConfig.partnerServiceId?.length === 8 ? "PSID siap" : "Isi PSID") : "Siap dikonfigurasi"}</td><td><Toggle checked={channel.enabled} onChange={(checked) => setChannels((current) => current.map((item) => item.method === channel.method && item.id === channel.id ? { ...item, enabled: checked } : item))} /></td><td className="pr-4 text-right"><button type="button" onClick={() => setEditChannel(channel)} className={buttonClass}>Edit</button></td></tr>)}</tbody>
        </table>
      </Panel>
      <div className="space-y-4">
        <Panel title="Master Gateway" description="Master OFF menyembunyikan seluruh metode milik gateway tersebut."><div className="space-y-3 p-4">
          <GatewayToggle title="Midtrans BI-SNAP" detail="Virtual Account bank" ready={Boolean(gatewayReadiness.midtrans?.ready)} enabled={gatewayActive("midtrans")} onChange={(value) => setGatewaySettings((current) => current.map((item) => item.gateway === "midtrans" ? { ...item, isActive: value } : item))} />
          <GatewayToggle title="DOKU Direct API" detail="QRIS dan E-Wallet" ready={Boolean(gatewayReadiness.doku?.ready)} enabled={gatewayActive("doku")} onChange={(value) => setGatewaySettings((current) => current.map((item) => item.gateway === "doku" ? { ...item, isActive: value } : item))} />
        </div></Panel>
        <Panel title="Aturan Routing" description="Tidak ada fallback otomatis antar gateway."><div className="space-y-2 p-4 text-[9px] text-[#42516a]"><div className="flex justify-between"><span>Virtual Account</span><strong>Midtrans BI-SNAP</strong></div><div className="flex justify-between"><span>QRIS</span><strong>DOKU Direct API</strong></div><div className="flex justify-between"><span>E-Wallet</span><strong>DOKU Direct API</strong></div></div></Panel>
        {canManageWallet && <Panel title="Top Up Saldo" description="Pengaturan wallet lama tetap terpisah dari split checkout toko."><div className="grid gap-3 p-4"><Field label="Minimum top up saldo"><input className={inputClass} inputMode="numeric" value={walletSettings.minTopup} onChange={(event) => setWalletSettings((current) => ({ ...current, minTopup: Number(event.target.value.replace(/\D/g, "")) || 0 }))} /></Field><div className="flex items-center justify-between rounded-md border border-[#e3e8ef] p-3"><div><strong className="block text-[9px] text-[#34445f]">Top up wallet otomatis</strong><span className="text-[8px] text-[#8a98aa]">Saat ini tetap menggunakan jalur DOKU yang sudah ada</span></div><Toggle checked={walletSettings.dokuTopupEnabled} onChange={(value) => setWalletSettings((current) => ({ ...current, dokuTopupEnabled: value }))} /></div></div></Panel>}
        <Panel title="Callback Publik" description="URL untuk dashboard gateway."><div className="space-y-2 p-4"><CopyUrl label="DOKU Notification" value="https://lfamiliastore.my.id/api/payments/doku/callback" /><CopyUrl label="Midtrans VA Notification" value="https://lfamiliastore.my.id/api/payments/midtrans/v1.0/transfer-va/payment" /></div></Panel>
      </div>
    </div>}
    ...
    {tab === "Transaksi Pembayaran" && <Panel title="Transaksi Pembayaran Terbaru" description="Riwayat pembayaran lintas gateway yang tersimpan di database." action={<div className="relative"><Search className="absolute left-2.5 top-1/2 size-3.5 -translate-y-1/2 text-[#8190a5]" /><input value={transactionQuery} onChange={(event) => setTransactionQuery(event.target.value)} className={`${inputClass} w-56 pl-8`} placeholder="Cari invoice atau pelanggan..." /></div>}><table className="w-full text-left"><thead className="bg-[#f6f8fb] text-[8px] uppercase text-[#718198]"><tr>{["Invoice", "Pelanggan", "Channel", "Total", "Status", "Waktu", "Aksi"].map((head) => <th key={head} className="px-4 py-2.5">{head}</th>)}</tr></thead><tbody className="divide-y divide-[#edf0f4]">{shownTransactions.map((order) => <tr key={order.id} className="text-[9px] text-[#42516a]"><td className="px-4 py-2.5"><strong className="text-[#0769e9]">{order.reference_id}</strong></td><td className="px-4">{order.buyer_name}</td><td className="px-4">{order.payment_channel}</td><td className="px-4">{formatMoney(order.total)}</td><td className="px-4"><Status tone={order.payment_status === "paid" ? "green" : order.payment_status === "pending" ? "amber" : "red"}>{order.payment_status}</Status></td><td className="px-4">{formatDate(order.created_at)}</td><td className="px-4"><button type="button" onClick={() => setSelectedTransaction(order)} className={buttonClass}><Eye className="size-3.5" />Detail</button></td></tr>)}</tbody></table></Panel>}

    <Modal open={Boolean(editChannel)} title={`Edit ${editChannel?.name || "Metode"}`} description="Pengaturan internal metode pembayaran. Nama gateway tidak ditampilkan kepada customer." onClose={() => { setEditChannel(null); setChannelFile(null); }} footer={<><button type="button" className={buttonClass} onClick={() => { setEditChannel(null); setChannelFile(null); }}>Batal</button><button type="button" disabled={busy} className={primaryButtonClass} onClick={saveEditedChannel}>{busy ? "Menyimpan..." : "Simpan Metode"}</button></>}><div className="grid grid-cols-2 gap-4"><Field label="Nama tampilan"><input className={inputClass} value={editChannel?.name || ""} onChange={(event) => setEditChannel((current) => current ? { ...current, name: event.target.value } : current)} /></Field><Field label="Kode channel"><input className={inputClass} value={editChannel?.id.toUpperCase() || ""} readOnly /></Field><Field label="Gateway internal"><input className={inputClass} value={editChannel?.gateway === "midtrans" ? "Midtrans BI-SNAP" : "DOKU Direct API"} readOnly /></Field><Field label="Urutan"><input className={inputClass} type="number" value={editChannel?.sortOrder ?? 0} onChange={(event) => setEditChannel((current) => current ? { ...current, sortOrder: Number(event.target.value) || 0 } : current)} /></Field><Field label="Deskripsi pelanggan" wide><input className={inputClass} value={editChannel?.description || ""} onChange={(event) => setEditChannel((current) => current ? { ...current, description: event.target.value } : current)} /></Field>{editChannel?.gateway === "midtrans" && <Field label="Partner Service ID VA" help="Harus tepat 8 karakter. Spasi di sebelah kiri dari Midtrans harus dipertahankan." wide><input className={`${inputClass} font-mono`} value={editChannel.gatewayConfig.partnerServiceId || ""} onChange={(event) => setEditChannel((current) => current ? { ...current, gatewayConfig: { ...current.gatewayConfig, partnerServiceId: event.target.value } } : current)} placeholder="   12345" maxLength={8} /></Field>}<Field label="Logo channel" wide><label className="flex h-20 cursor-pointer items-center justify-center gap-2 rounded-md border border-dashed border-[#b9c8db] bg-[#f8fafc] text-[9px] font-bold text-[#52627a]"><ImagePlus className="size-4 text-[#0769e9]" />{channelFile || editChannel?.imageUrl ? "Ganti gambar logo" : "Pilih gambar logo"}<input type="file" accept="image/png,image/jpeg,image/webp" className="hidden" onChange={(event) => { const file = event.target.files?.[0]; if (!file) return; if (file.size > 2 * 1024 * 1024) { setError("Ukuran gambar maksimal 2MB."); return; } setChannelFile(file); const reader = new FileReader(); reader.onload = () => setEditChannel((current) => current ? { ...current, imageUrl: String(reader.result || "") } : current); reader.readAsDataURL(file); }} /></label></Field></div></Modal>
...
function GatewayToggle({ title, detail, ready, enabled, onChange }: { title: string; detail: string; ready: boolean; enabled: boolean; onChange(value: boolean): void }) { return <div className="rounded-md border border-[#e3e8ef] p-3"><div className="mb-2 flex items-center justify-between"><div><strong className="block text-[9px] text-[#34445f]">{title}</strong><span className="text-[8px] text-[#8a98aa]">{detail}</span></div><Toggle checked={enabled} onChange={onChange} /></div><Status tone={ready ? "green" : "amber"}>{ready ? "Konfigurasi Siap" : "Belum Siap"}</Status></div>; }
