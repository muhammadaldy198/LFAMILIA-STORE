  ...
  return <div>
    ...
    {tab === "Metode Pembayaran" && <div className="grid grid-cols-[minmax(0,1fr)_340px] gap-4">
      <Panel title="Metode Pembayaran" description="Tambah, hapus, aktif/nonaktifkan, dan tentukan gateway tiap metode dari sini." action={<div className="flex items-center gap-2"><button type="button" onClick={openCreateChannel} disabled={busy} className={primaryButtonClass}><Plus className="size-3.5" />Tambah Metode</button><button type="button" onClick={syncChannels} disabled={busy} className={buttonClass}><RefreshCw className="size-3.5" />Sinkron Provider</button></div>}>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[760px] text-left"><thead className="bg-[#f6f8fb] text-[8px] uppercase text-[#718198]"><tr><th className="px-4 py-2.5">Channel</th><th>Jenis</th><th>Gateway</th><th>Kesiapan</th><th>Status</th><th className="pr-4 text-right">Aksi</th></tr></thead>
            <tbody className="divide-y divide-[#edf0f4]">{channels.map((channel) => <tr key={`${channel.dbId ?? "new"}:${channel.method}:${channel.id}`} className="text-[9px] text-[#42516a]"><td className="px-4 py-2.5"><div className="flex items-center gap-2"><span className="grid size-7 place-items-center rounded bg-blue-50 text-[#0769e9]">{channel.group === "QRIS" ? <QrCode className="size-3.5" /> : channel.group === "VA Bank" ? <Landmark className="size-3.5" /> : <Smartphone className="size-3.5" />}</span><div><strong className="block text-[#23334e]">{channel.name}</strong><span className="text-[8px] text-[#8a98aa]">{channel.description || channel.id}</span></div></div></td><td>{channel.group}</td><td><select className={`${inputClass} h-8 w-28`} value={channel.gateway} onChange={(event) => setChannels((current) => current.map((item) => item.dbId === channel.dbId ? { ...item, gateway: event.target.value as Gateway } : item))}><option value="doku">DOKU</option><option value="midtrans">Midtrans</option></select></td><td><Status tone={gatewayReadiness[channel.gateway]?.ready ? "green" : "amber"}>{gatewayReadiness[channel.gateway]?.ready ? "Siap" : "Belum siap"}</Status></td><td><Toggle checked={channel.enabled} onChange={(checked) => setChannels((current) => current.map((item) => item.dbId === channel.dbId ? { ...item, enabled: checked } : item))} /></td><td className="pr-4 text-right"><div className="flex justify-end gap-1.5"><button type="button" onClick={() => { setIsCreatingChannel(false); setEditChannel(channel); }} className={buttonClass}>Edit</button><button type="button" onClick={() => deleteChannel(channel)} className={`${buttonClass} text-rose-600`}><Trash2 className="size-3.5" />Hapus</button></div></td></tr>)}</tbody>
          </table>
      ...
      <div className="space-y-4">
        <Panel title="Gateway & Environment" description="Mode gateway dan environment dipilih di sini; tidak ada panel routing terpisah."><div className="space-y-3 p-4">
          <GatewayControl gateway="doku" title="DOKU" ready={Boolean(gatewayReadiness.doku?.ready)} enabled={gatewayActive("doku")} mode={routing?.dokuMode || "checkout"} environment={routing?.dokuEnvironment || "sandbox"} onEnabled={(value) => setGatewaySettings((current) => current.map((item) => item.gateway === "doku" ? { ...item, isActive: value } : item))} onMode={(value) => setRouting((current) => current ? { ...current, dokuMode: value as RoutingOverview["dokuMode"] } : current)} onEnvironment={(value) => setRouting((current) => current ? { ...current, dokuEnvironment: value } : current)} />
          <GatewayControl gateway="midtrans" title="Midtrans" ready={Boolean(gatewayReadiness.midtrans?.ready)} enabled={gatewayActive("midtrans")} mode={routing?.midtransMode || "snap"} environment={routing?.midtransEnvironment || "sandbox"} onEnabled={(value) => setGatewaySettings((current) => current.map((item) => item.gateway === "midtrans" ? { ...item, isActive: value } : item))} onMode={(value) => setRouting((current) => current ? { ...current, midtransMode: value as RoutingOverview["midtransMode"] } : current)} onEnvironment={(value) => setRouting((current) => current ? { ...current, midtransEnvironment: value } : current)} />
        </div></Panel>
        {canManageWallet && <Panel title="Top Up Saldo" description="Top up saldo memakai daftar metode dan routing gateway yang sama dengan checkout."><div className="grid gap-3 p-4"><Field label="Minimum top up saldo"><input className={inputClass} inputMode="numeric" value={walletSettings.minTopup} onChange={(event) => setWalletSettings((current) => ({ ...current, minTopup: Number(event.target.value.replace(/\D/g, "")) || 0 }))} /></Field><div className="flex items-center justify-between rounded-md border border-[#e3e8ef] p-3"><div><strong className="block text-[9px] text-[#34445f]">Top up wallet otomatis</strong><span className="text-[8px] text-[#8a98aa]">Metode aktif mengikuti konfigurasi pada tabel di sebelah kiri.</span></div><Toggle checked={walletSettings.dokuTopupEnabled} onChange={(value) => setWalletSettings((current) => ({ ...current, dokuTopupEnabled: value }))} /></div></div></Panel>}
        <Panel title="Callback Publik" description="URL untuk dashboard gateway."><div className="space-y-2 p-4"><CopyUrl label="DOKU Notification" value="https://lfamiliastore.my.id/api/payments/doku/callback" /><CopyUrl label="Midtrans Snap Notification" value="https://lfamiliastore.my.id/api/payments/midtrans/snap/notification" /><CopyUrl label="Midtrans BI-SNAP VA Notification" value="https://lfamiliastore.my.id/api/payments/midtrans/v1.0/transfer-va/payment" /></div></Panel>
      </div>
    </div>}
    ...
    <Modal open={Boolean(editChannel)} title={`${isCreatingChannel ? "Tambah" : "Edit"} ${editChannel?.name || "Metode Pembayaran"}`} description="Metode dan gateway dapat dikelola dari Admin. Kode gateway opsional hanya dipakai jika channel belum ada pada mapping bawaan provider." onClose={() => { setEditChannel(null); setIsCreatingChannel(false); setChannelFile(null); }} footer={<><button type="button" className={buttonClass} onClick={() => { setEditChannel(null); setIsCreatingChannel(false); setChannelFile(null); }}>Batal</button><button type="button" disabled={busy} className={primaryButtonClass} onClick={saveEditedChannel}>{busy ? "Menyimpan..." : "Simpan Metode"}</button></>}><div className="grid grid-cols-2 gap-4">
      ...
      <Field label="Gateway"><select className={inputClass} value={editChannel?.gateway || "midtrans"} onChange={(event) => setEditChannel((current) => current ? { ...current, gateway: event.target.value as Gateway } : current)}><option value="doku">DOKU</option><option value="midtrans">Midtrans</option></select></Field>
      ...
      <Field label="Biaya dibebankan ke customer (%)" help="Semua biaya gateway untuk channel ini dihitung di backend dan ditambahkan ke total customer. QRIS bawaan 0,7%."><input className={inputClass} type="number" min="0" max="100" step="0.01" value={String((Number(editChannel?.gatewayConfig.customerFeeBps ?? (editChannel?.method === "qris" ? 70 : 0)) || 0) / 100)} onChange={(event) => setEditChannel((current) => current ? { ...current, gatewayConfig: { ...current.gatewayConfig, customerFeeBps: String(Math.max(0, Math.min(10_000, Math.round(Number(event.target.value || 0) * 100))) } } : current)} placeholder="0.7" /></Field>
      ...
      {editChannel?.gateway === "midtrans" && <Field label="Partner Service ID VA" help="Hanya diperlukan saat memakai Midtrans BI-SNAP VA." wide><input className={`${inputClass} font-mono`} value={editChannel.gatewayConfig.partnerServiceId || ""} onChange={(event) => setEditChannel((current) => current ? { ...current, gatewayConfig: { ...current.gatewayConfig, partnerServiceId: event.target.value } } : current)} placeholder="   12345" maxLength={8} /></Field>}
...
function GatewayControl({ gateway, title, ready, enabled, mode, environment, onEnabled, onMode, onEnvironment }: { gateway: Gateway; title: string; ready: boolean; enabled: boolean; mode: string; environment: Environment; onEnabled(value: boolean): void; onMode(value: string): void; onEnvironment(value: Environment): void }) {
  const leftMode = gateway === "doku" ? "checkout" : "snap";
  const rightMode = gateway === "doku" ? "direct" : "bisnap";
  const leftLabel = gateway === "doku" ? "Checkout Biasa" : "Snap";
  const rightLabel = gateway === "doku" ? "Direct API" : "BI-SNAP";
  return <div className="rounded-md border border-[#e3e8ef] p-3">
    ...
    <div className="space-y-2">
      <div><span className="mb-1 block text-[8px] font-bold text-[#52627a]">Mode Gateway</span><div className="flex items-center justify-between rounded-md border border-[#e3e8ef] bg-[#f8fafc] px-3 py-2"><span className={`text-[8px] ${mode === leftMode ? "font-extrabold text-[#1769e8]" : "font-semibold text-[#8190a5]"}`}>{leftLabel}</span><Toggle checked={mode === rightMode} onChange={(checked) => onMode(checked ? rightMode : leftMode)} /><span className={`text-[8px] ${mode === rightMode ? "font-extrabold text-[#1769e8]" : "font-semibold text-[#8190a5]"}`}>{rightLabel}</span></div></div>
