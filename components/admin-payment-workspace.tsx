  ...
  return <div>
    ...
    {tab === "Metode Pembayaran" && <div className="grid grid-cols-[minmax(0,1fr)_340px] gap-4">
      ...
      <div className="space-y-4">
        <Panel title="Gateway & Environment" description="Pengaturan ini menggantikan panel routing terpisah. Semua tetap di menu Metode Pembayaran."><div className="space-y-3 p-4">
          <GatewayControl gateway="doku" title="DOKU" ready={Boolean(gatewayReadiness.doku?.ready)} enabled={gatewayActive("doku")} mode={routing?.dokuMode || "checkout"} environment={routing?.dokuEnvironment || "sandbox"} modeOptions={[{ value: "checkout", label: "Checkout" }, { value: "direct", label: "Direct API" }]} onEnabled={(value) => setGatewaySettings((current) => current.map((item) => item.gateway === "doku" ? { ...item, isActive: value } : item))} onMode={(value) => setRouting((current) => current ? { ...current, dokuMode: value as RoutingOverview["dokuMode"] } : current)} onEnvironment={(value) => setRouting((current) => current ? { ...current, dokuEnvironment: value } : current)} />
          <GatewayControl gateway="midtrans" title="Midtrans" ready={Boolean(gatewayReadiness.midtrans?.ready)} enabled={gatewayActive("midtrans")} mode={routing?.midtransMode || "snap"} environment={routing?.midtransEnvironment || "sandbox"} modeOptions={[{ value: "snap", label: "Snap" }, { value: "bisnap", label: "BI-SNAP" }]} onEnabled={(value) => setGatewaySettings((current) => current.map((item) => item.gateway === "midtrans" ? { ...item, isActive: value } : item))} onMode={(value) => setRouting((current) => current ? { ...current, midtransMode: value as RoutingOverview["midtransMode"] } : current)} onEnvironment={(value) => setRouting((current) => current ? { ...current, midtransEnvironment: value } : current)} />
        </div></Panel>
...
function GatewayControl({ title, ready, enabled, mode, environment, modeOptions, onEnabled, onMode, onEnvironment }: { gateway: Gateway; title: string; ready: boolean; enabled: boolean; mode: string; environment: Environment; modeOptions: Array<{ value: string; label: string }>; onEnabled(value: boolean): void; onMode(value: string): void; onEnvironment(value: Environment): void }) {
  return <div className="rounded-md border border-[#e3e8ef] p-3"><div className="mb-2 flex items-center justify-between"><div><strong className="block text-[9px] text-[#34445f]">{title}</strong><Status tone={ready ? "green" : "amber"}>{ready ? "Konfigurasi Siap" : "Belum Siap"}</Status></div><Toggle checked={enabled} onChange={onEnabled} /></div><div className="grid grid-cols-2 gap-2"><Field label="Mode"><select className={inputClass} value={mode} onChange={(event) => onMode(event.target.value)}>{modeOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></Field><Field label="Environment"><select className={inputClass} value={environment} onChange={(event) => onEnvironment(event.target.value as Environment)}><option value="sandbox">Sandbox</option><option value="production">Production</option></select></Field></div></div>;
}
