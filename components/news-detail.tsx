...
export function NewsDetail() {
  ...
  useEffect(() => { if (!slug) return; void fetch(`/api/news?slug=${encodeURIComponent(slug)}`, { cache: "no-store" }).then(async (response) => response.ok ? response.json() : null).then((data: { article?: NewsRecord } | null) => setArticle(data?.article ?? null)).finally(() => setLoading(false)); }, [slug]);
