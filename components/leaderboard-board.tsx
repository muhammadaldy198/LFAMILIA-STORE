...
export function LeaderboardBoard() {
  ...
  useEffect(() => { void Promise.all((["month", "all"] as const).map(async (period) => { const response = await fetch(`/api/leaderboard?period=${period}`, { cache: "no-store" }); const result = await response.json() as { entries?: Entry[] }; return [period, result.entries ?? []] as const; })).then((values) => setData(Object.fromEntries(values) as Record<"month" | "all", Entry[]>)).finally(() => setLoading(false)); }, []);
