  ...
  if (id) {
    const result = await db.prepare(
      `UPDATE flash_sales
       ...
       WHERE id = ?
         AND (
           (product_slug = ? AND package_sku = ?)
           OR (
             reserved_count = 0
             AND sold_count = 0
             AND NOT EXISTS (
               SELECT 1
               FROM promotion_reservations history
               WHERE history.flash_sale_id = flash_sales.id
             )
           )
         )
    ...
    if (Number(result.meta?.changes ?? 0) === 0) {
      ...
      if (changingIdentity) {
        ...
        if (current.reserved_count > 0 || current.sold_count > 0 || historical) {
          throw new Error("Produk/nominal flash sale tidak dapat diganti setelah promo pernah digunakan, direservasi, atau memiliki riwayat transaksi.");
        }
      }
