...
export async function saveIntegrationProfile(input: {
  ...
  const ownership = guardedOwnershipClause();
  const guarded = Boolean(guardToken);
  const committed = guarded
    ...
    : await database.prepare(`INSERT INTO integration_profiles (provider, mode, environment, encrypted_config, updated_at)
        ...
        .first<{ encrypted_config: string; created_at: string; updated_at: string }>();
  if (!committed) {
    if (guarded) throw new Error("Guard konfigurasi DigiFlazz kedaluwarsa sebelum profile dapat disimpan.");
...
export async function saveIntegrationSelections(
  ...
  if (!committed) {
    if (guarded) throw new Error("Guard konfigurasi DigiFlazz kedaluwarsa sebelum environment dapat disimpan.");
