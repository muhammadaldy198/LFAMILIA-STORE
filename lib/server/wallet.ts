...
export async function readWalletSettings(): Promise<WalletSettings> {
  try {
    ...
    const row = await getD1().prepare("SELECT * FROM wallet_settings WHERE id = 1").first<{
      ...
      manual_qris_enabled: number; manual_qris_name: string; manual_qris_image_url: string | null; ipaymu_topup_enabled: number;
    }>();
...
export async function saveWalletSettings(input: WalletSettings) {
  await ensureLegacyDatabaseColumns();
  await getD1().prepare(
    `INSERT INTO wallet_settings (id, is_enabled, method_name, account_name, account_number, instructions, min_topup, manual_qris_enabled, manual_qris_name, manual_qris_image_url, ipaymu_topup_enabled, updated_at)
     ...
     ON CONFLICT(id) DO UPDATE SET is_enabled = excluded.is_enabled, method_name = excluded.method_name,
      ...
      manual_qris_image_url = excluded.manual_qris_image_url, ipaymu_topup_enabled = excluded.ipaymu_topup_enabled,
      updated_at = CURRENT_TIMESTAMP`,
  ).bind(input.isEnabled ? 1 : 0, input.methodName, input.accountName, input.accountNumber, input.instructions, input.minTopup, input.manualQrisEnabled ? 1 : 0, input.manualQrisName, input.manualQrisImageUrl || null, input.ipaymuTopupEnabled ? 1 : 0).run();
}
...
export async function createIpaymuWalletTopup(input: { customerId: string; amount: number; name: string; paymentMethod: string; paymentChannel: string; referenceId: string }) {
  await ensureLegacyDatabaseColumns();
  const id = crypto.randomUUID();
  await getD1().prepare(
    `INSERT INTO wallet_topups (id, customer_id, amount, sender_name, payment_method, proof_url, source, reference_id)
     VALUES (?, ?, ?, ?, ?, '', 'ipaymu', ?)`,
  ).bind(id, input.customerId, input.amount, input.name, `${input.paymentMethod}:${input.paymentChannel}`, input.referenceId).run();
  return id;
}

export async function updateIpaymuWalletTopup(input: { referenceId: string; transactionId: string | null; paymentNo: string | null; paymentName: string | null; paymentUrl: string | null; expiredAt: string | null; fee: number; total: number }) {
  await getD1().prepare(
...
export async function getIpaymuWalletTopup(referenceId: string) {
  ...
  ).bind(referenceId).first<{ id: string; customer_id: string; amount: number; status: string; ipaymu_transaction_id: string | null }>();
}

export async function applyIpaymuWalletTopup(input: { referenceId: string; status: "paid" | "pending" | "expired" | "failed"; transactionId: string | null; callbackAmount: number }) {
  const topup = await getIpaymuWalletTopup(input.referenceId);
  if (!topup) return { found: false, credited: false };
  if (topup.ipaymu_transaction_id && input.transactionId && topup.ipaymu_transaction_id !== input.transactionId) return { found: true, credited: false, ignored: "transaction_mismatch" };
  if (input.status === "paid" && input.callbackAmount > 0 && input.callbackAmount < topup.amount) return { found: true, credited: false, ignored: "amount_mismatch" };
  const db = getD1();
  if (input.status === "paid") {
    ...
    await db.batch([
      ...
      ).bind(crypto.randomUUID(), reference, `Top up otomatis iPaymu ${topup.id.slice(0, 8).toUpperCase()}`, topup.customer_id, topup.id),
      db.prepare("UPDATE wallet_topups SET status = 'approved', reviewed_by = 'ipaymu-callback', reviewed_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND status = 'pending'").bind(topup.id),
      db.prepare("UPDATE customer_users SET balance = (SELECT COALESCE(SUM(CASE WHEN direction = 'credit' THEN amount ELSE -amount END), 0) FROM wallet_transactions WHERE customer_id = ?), updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(topup.customer_id, topup.customer_id),
    ]);
  ...
  if (input.status === "expired" || input.status === "failed") {
    await db.prepare("UPDATE wallet_topups SET status = 'rejected', admin_notes = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND status = 'pending'")
      .bind(input.status === "expired" ? "Pembayaran iPaymu kedaluwarsa." : "Pembayaran iPaymu gagal.", topup.id).run();
  }
