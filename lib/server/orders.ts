...
export async function resolvePurchasableItem(productSlug: string, packageSku: string): Promise<PurchasableItem | null> {
  const db = getD1();
  const row = await db.prepare(
    `SELECT p.slug AS product_slug, p.name AS product_name, p.needs_server,
     ...
     LIMIT 1`,
  ).bind(productSlug, packageSku).first<StoredItemRow>();
  if (row) {
    return {
  ...
  const fallback = getFallbackProducts().find((item) => item.slug === productSlug);
...
export function renderCustomerNo(template: string, destination: string, server: string | null) {
  const value = template
    .replaceAll("{{destination}}", destination.trim())
    .replaceAll("{{server}}", server?.trim() ?? "")
    .trim();
  if (!value || value.includes("{{") || value.length > 120) throw new Error("Format tujuan provider belum valid.");
  return value;
}
...
export function createOrderIdentity() {
  ...
  return { id, referenceId: `LF-${date}-${id.replaceAll("-", "").slice(0, 12).toUpperCase()}` };
}
...
export async function insertPendingOrder(input: {
  ...
  if (input.item.fulfillmentType === "automatic") {
    if (!input.item.providerCode || !input.item.providerSku) throw new Error("Provider dan SKU produk otomatis belum diatur oleh admin.");
    if (!getProviderAdapter(input.item.providerCode)) throw new Error(`Adapter provider ${input.item.providerCode} belum tersedia.`);
  }
...
export async function getOrderByReference(referenceId: string) {
  return getD1().prepare("SELECT * FROM orders WHERE reference_id = ? LIMIT 1").bind(referenceId).first<OrderRecord>();
}

export async function getOrderById(id: string) {
  return getD1().prepare("SELECT * FROM orders WHERE id = ? LIMIT 1").bind(id).first<OrderRecord>();
}
...
export async function updateIpaymuPayment(input: {
  ...
  await getD1().prepare(
    `UPDATE orders SET ipaymu_transaction_id = ?, ipaymu_payment_no = ?, ipaymu_payment_name = ?,
     ...
     WHERE reference_id = ?`,
  ).bind(input.transactionId, input.paymentNo, input.paymentName, input.paymentUrl, input.expiredAt, input.fee, input.total, input.referenceId).run();
}

export async function markPaymentCreationFailed(referenceId: string, message: string) {
  await getD1().prepare(
...
export async function recordOrderEvent(input: {
  ...
  return getD1().prepare(
    `INSERT OR IGNORE INTO order_events (order_id, source, event_id, status, payload_json)
     VALUES (?, ?, ?, ?, ?)`,
  ).bind(input.orderId, input.source, input.eventId, input.status, JSON.stringify(input.payload)).run();
}

export async function applyPaymentStatus(order: OrderRecord, status: "paid" | "pending" | "expired" | "failed") {
  const db = getD1();
  if (status === "paid") {
...
export async function fulfillAutomaticOrder(orderId: string, publicBaseUrl: string) {
  const order = await getOrderById(orderId);
  if (!order || order.payment_status !== "paid" || order.fulfillment_type !== "automatic") return;
  if (!order.provider_code || !order.provider_sku || !order.customer_no) {
    await setFulfillmentError(order.id, "Provider, SKU, atau format tujuan belum lengkap.");
    return;
  }
  const adapter = getProviderAdapter(order.provider_code);
  if (!adapter) {
    await setFulfillmentError(order.id, `Adapter ${order.provider_code} belum tersedia.`);
    return;
  }

  if (order.provider_code === "digiflazz" && !order.provider_ref_id) {
    await getD1().prepare("UPDATE orders SET provider_ref_id = ? WHERE id = ?").bind(order.reference_id, order.id).run();
  }
  ...
  try {
    ...
    await setFulfillmentError(order.id, error instanceof Error ? error.message : "Provider gagal dihubungi.");
  }
}

async function applyProviderResult(order: OrderRecord, result: ProviderResult, eventId?: string) {
  const db = getD1();
  await db.prepare(
  ...
  ).bind(result.externalId, result.status, result.message, result.serialNumber, result.status, order.id).run();
  await recordOrderEvent({
    orderId: order.id,
    source: order.provider_code === "digiflazz" ? "digiflazz" : order.provider_code === "vippayment" ? "vippayment" : order.provider_code === "voucher-stock" ? "voucher_stock" : "admin",
...
export async function applyProviderWebhook(input: {
  ...
  ).bind(input.providerCode, input.providerRefId, input.providerRefId).first<OrderRecord>();
...
export async function listOrders(limit = 200) {
  ...
  ).bind(Math.min(Math.max(limit, 1), 500)).all<OrderRecord>();
...
export async function completeManualOrder(id: string, adminEmail: string) {
  ...
  if (!order || order.fulfillment_type !== "manual" || order.payment_status !== "paid") {
  ...
  await db.prepare(
    `UPDATE orders SET fulfillment_status = 'success', provider_status = 'manual_done',
     provider_message = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`,
  ).bind(`Diselesaikan oleh ${adminEmail}`, id).run();
  await recordOrderEvent({ orderId: id, source: "admin", eventId: `manual-${id}-${Date.now()}`, status: "success", payload: { adminEmail } });
}
