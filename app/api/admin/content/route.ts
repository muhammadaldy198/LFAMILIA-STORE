...
const bannerSchema = z.object({ id: z.number().int().positive().nullable().optional(), title: z.string().trim().min(2).max(120), subtitle: z.string().trim().max(300), imageUrl: mediaUrl, ctaLabel: z.string().trim().min(2).max(50), ctaHref: z.string().trim().min(1).max(300), isActive: z.boolean(), sortOrder: z.number().int().min(0).max(10000) });
