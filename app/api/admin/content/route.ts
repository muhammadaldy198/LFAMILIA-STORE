...
const bannerSchema = z.object({ id: z.number().int().positive().nullable().optional(), title: z.string().trim().min(2).max(120), subtitle: z.string().trim().max(300), imageUrl: mediaUrl, mobileImageUrl: mediaUrl.optional().or(z.literal("")).default(""), ctaLabel: z.string().trim().max(50).default(""), ctaHref: optionalUrl.default(""), showDesktop: z.boolean().default(true), showMobile: z.boolean().default(true), isActive: z.boolean(), sortOrder: z.number().int().min(0).max(10000) }).refine((item) => item.showDesktop || item.showMobile, { message: "Pilih minimal satu tampilan banner." });
