...
import { createIpaymuWalletTopup, createWalletTopup, readWalletSettings, updateIpaymuWalletTopup } from "@/lib/server/wallet";

type RuntimeEnv = { PUBLIC_BASE_URL?: string };

const automaticSchema = z.object({
  mode: z.literal("ipaymu"),
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (request.headers.get("content-type")?.includes("application/json")) {
      ...
      if (!settings.ipaymuTopupEnabled) throw new Error("Top up otomatis belum diaktifkan oleh Pemilik.");
      if (input.amount < settings.minTopup) throw new Error(`Minimum top up Rp${settings.minTopup.toLocaleString("id-ID")}.`);
      if (!await isPaymentChannelAvailable(input.paymentMethod, input.paymentChannel)) throw new Error("Metode iPaymu tidak tersedia.");
      ...
      try {
        const payment = await createIpaymuDirectPayment({
          ...
          notifyUrl: `${new URL(configured || request.url).origin}/api/payments/ipaymu/callback`,
        ...
        return Response.json({ ok: true, mode: "ipaymu", referenceId, paymentNo: payment.paymentNo, paymentName: payment.paymentName, paymentUrl: payment.paymentUrl, total: payment.total, fee: payment.fee, expiredAt: payment.expiredAt }, { status: 201 });
      } catch (error) {
        throw error;
      }
    }
    if (!settings.isEnabled || !settings.accountNumber) throw new Error("Top up manual bank belum dibuka oleh Pemilik.");
    ...
    if (paymentMethod === "QRIS Manual" && !settings.manualQrisEnabled) throw new Error("QRIS manual belum diaktifkan oleh Pemilik.");
