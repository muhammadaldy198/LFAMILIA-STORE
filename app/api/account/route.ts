...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    await getD1().prepare("UPDATE customer_users SET name = ?, phone = ?, phone_verified_at = NULL, leaderboard_opt_in = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?")
      .bind(input.name, normalizedPhone, input.leaderboardOptIn ? 1 : 0, customer.id).run();
    return Response.json({ ok: true });
  } catch (error) {
    const message = error instanceof z.ZodError ? error.issues[0]?.message : error instanceof Error ? error.message : "Profil gagal diperbarui.";
