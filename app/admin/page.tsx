...
const orders = [
  ["INV-240828-001", "Mobile Legends", "59 Diamonds", "Rp17.624", "Berhasil"],
  ["INV-240828-002", "Free Fire", "140 Diamonds", "Rp20.645", "Diproses"],
  ["INV-240828-003", "Steam Wallet", "Rp45.000", "Rp49.346", "Menunggu"],
  ["INV-240828-004", "PUBG Mobile", "325 UC", "Rp74.019", "Gagal"],
];
  ...
  return (
    <StoreLayout>
      <main className="mx-auto min-h-[75vh] max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-7 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><div className="flex items-center gap-2"><p className="eyebrow !mb-0">Panel pengelola</p><span className="rounded-full bg-amber-400/10 px-2 py-0.5 text-[8px] font-black uppercase text-amber-300">Demo frontend</span></div><h1 className="mt-2 text-3xl font-black tracking-[-0.04em] sm:text-4xl">Admin LFAMILIA STORE</h1><p className="mt-2 text-xs text-white/35">Contoh modul pengelolaan sebelum database dan API dipasang.</p></div><Button asChild variant="outline" className="w-fit rounded-xl border-white/10 bg-white/[0.035] text-white hover:bg-white/[0.08] hover:text-white"><Link href="/"><ShoppingBag className="mr-2 size-4" />Lihat toko</Link></Button></div>
        <Tabs defaultValue="overview" className="grid items-start gap-5 lg:grid-cols-[220px_1fr]">
          ...
          <div className="min-w-0">
            ...
            <TabsContent value="orders" className="mt-0"><AdminSection title="Daftar pesanan" description="Cari, pantau, dan perbarui transaksi pelanggan."><div className="mb-4 flex flex-col gap-3 sm:flex-row"><Input placeholder="Cari invoice atau produk..." className="h-10 rounded-xl border-white/10 bg-white/[0.035] text-white placeholder:text-white/22" /><select className="h-10 rounded-xl border border-white/10 bg-[#151924] px-3 text-xs text-white"><option>Semua status</option><option>Berhasil</option><option>Diproses</option><option>Menunggu</option><option>Gagal</option></select></div><DataTable headers={["Invoice", "Produk", "Item", "Total", "Status"]} rows={orders} /></AdminSection></TabsContent>
            <TabsContent value="products" className="mt-0"><AdminSection title="Katalog produk" description="Atur produk, margin, dan status penjualan." action="Tambah produk"><DataTable headers={["Produk", "Kategori", "Pemasok", "Varian", "Status"]} rows={products.slice(0, 7).map((item) => [item.name, item.category === "game" ? "Game" : "Voucher", "DigiFlazz", `${item.packages.length} varian`, "Aktif"])} /></AdminSection></TabsContent>
            <TabsContent value="vouchers" className="mt-0"><AdminSection title="Manajemen voucher" description="Kelola kode promo dan diskon pelanggan." action="Buat voucher"><DataTable headers={["Kode", "Diskon", "Minimum", "Penggunaan", "Status"]} rows={[["FAMILIA10", "10%", "Rp50.000", "18 / 100", "Aktif"], ["WELCOME5", "Rp5.000", "Rp25.000", "42 / 250", "Aktif"], ["HEMATQRIS", "2%", "Rp10.000", "—", "Draf"]]} /></AdminSection></TabsContent>
            ...
            <TabsContent value="settings" className="mt-0"><AdminSection title="Pengaturan toko" description="Konfigurasi identitas toko dan integrasi."><div className="grid gap-4 sm:grid-cols-2"><SettingCard title="DigiFlazz" status="Belum terhubung" text="API pemasok dan callback pesanan." /><SettingCard title="Midtrans" status="Belum terhubung" text="QRIS dan notifikasi pembayaran." /><SettingCard title="Biaya admin pelanggan" status="Simulasi 0,7%" text="Nilai final menunggu skema resmi Midtrans." /><SettingCard title="Mode pemeliharaan" status="Nonaktif" text="Batasi transaksi saat sistem diperbarui." /></div></AdminSection></TabsContent>
          </div>
        </Tabs>
      </main>
    </StoreLayout>
