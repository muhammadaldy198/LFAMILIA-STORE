        ...
        <div className="mt-6 grid items-start gap-6 lg:grid-cols-[1fr_380px]">
          <form id="checkout-form" onSubmit={submitOrder} className="space-y-5">
            ...
            <section className="panel p-5 sm:p-6">
              ...
              <div className="mt-5 space-y-3">{checkoutGroups.map((group) => {
                const Icon = groupIcons[group.code];
                const selected = paymentMethod === group.code;
                const disabled = group.code === "wallet" && !account;
                return <div key={group.code} className={`overflow-hidden rounded-2xl border transition ${selected ? "border-[#b9ff35]/60 bg-[#b9ff35]/[0.06]" : "border-white/[0.09] bg-white/[0.025]"}`}><button type="button" disabled={disabled} onClick={() => chooseMethod(group.code)} className="flex w-full items-center gap-3 p-4 text-left disabled:cursor-not-allowed disabled:opacity-45"><span className={`grid size-10 place-items-center rounded-xl ${selected ? "bg-[#b9ff35] text-[#091006]" : "bg-white/[0.06] text-white/55"}`}><Icon className="size-4" /></span><span className="min-w-0 flex-1"><strong className="block text-xs">{group.name}</strong><span className="mt-1 block text-[9px] text-white/35">{group.code === "wallet" && account ? `Saldo ${formatRupiah(account.balance)}` : group.code === "wallet" ? "Masuk akun untuk memakai saldo" : group.description}</span></span>{selected && <CheckCircle2 className="size-4 text-[#b9ff35]" />}</button>{selected && group.code !== "wallet" && <div className="grid grid-cols-2 gap-2 border-t border-white/[0.08] bg-black/10 p-3 sm:grid-cols-3">{availableChannels.filter((channel) => channel.method === group.code).map((channel) => <button key={channel.channel} type="button" onClick={() => { setPaymentChannel(channel.channel); setPayment(null); }} className={`flex min-h-14 items-center gap-2 rounded-xl border px-3 py-2 text-left text-[10px] font-bold transition ${paymentChannel === channel.channel ? "border-[#b9ff35]/60 bg-[#b9ff35]/[0.10] text-[#d8ff8d]" : "border-white/[0.08] bg-white/[0.02] text-white/45 hover:text-white"}`}>{channel.imageUrl ? <img src={channel.imageUrl} alt="" className="size-8 rounded-lg object-contain" /> : <span className="grid size-8 place-items-center rounded-lg bg-white/[0.07] text-[8px] font-black text-white/80">{channel.name.slice(0, 3)}</span>}<span className="truncate">{channel.name}</span></button>)}</div>}</div>;
              })}</div>
